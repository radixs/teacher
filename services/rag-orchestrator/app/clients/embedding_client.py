from __future__ import annotations

from typing import Any, Dict, List

import httpx

from ..core.flow_logger import log_flow
from ..services.embedding_generation_error import EmbeddingGenerationError


class EmbeddingClient:
    def __init__(self, base_url: str) -> None:
        self._client = httpx.AsyncClient(base_url=base_url, timeout=60.0)

    async def embed(self, text: str) -> List[float]:
        log_flow(
            "embedding-worker",
            "embed.requested",
            "RAG orchestrator requested an embedding for learner text.",
            text=text,
            text_length=len(text),
        )
        try:
            response = await self._client.post("/embed", json={"text": text})
            response.raise_for_status()
            embedding_response: Dict[str, Any] = response.json()
        except Exception as exc:
            log_flow(
                "embedding-worker",
                "embed.failed",
                "Embedding request failed and the orchestrator cannot continue without a valid vector response.",
                text_length=len(text),
                error=str(exc),
            )
            raise EmbeddingGenerationError(
                "Embedding worker failed to generate an embedding."
            ) from exc

        vector = embedding_response.get("embedding")
        if not isinstance(vector, list) or not vector:
            raise EmbeddingGenerationError(
                "Embedding worker returned no usable embedding vector."
            )

        log_flow(
            "embedding-worker",
            "embed.received",
            "RAG orchestrator received an embedding vector from the embedding worker.",
            dimensions=len(vector),
            model=embedding_response.get("model"),
        )
        return vector

    async def close(self) -> None:
        await self._client.aclose()
